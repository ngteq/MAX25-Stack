...
**HyBBX INI:** merge `share/hybbx/tnc2c-edge.ini.example` into Secondary `hybbx.ini`. Key fields: `tnc=tnc2c`, `protocol=kiss`, `device=/dev/ttyUSB0`, `baud`, `serial_line`, `rts_dtr`.
...
2. Merge matching `share/hybbx/*-edge.ini.example`
3. Build HyBBX with required plugin enabled
4. Start HyBBX Secondary → HBX link to Main

Operating mode plugin: `plugins/betriebsform/hybbx-edge/`.
...
- [LINUX-EDGE-SETUP.md](LINUX-EDGE-SETUP.md) — edge node install
